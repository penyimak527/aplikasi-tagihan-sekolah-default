...
class M_tarif_tagihan extends CI_Model
{
    ...
    public function simpan_kelas(){
        $id=(int)$this->input->post('id_tagihan');$tarif=$this->input->post('tarif');if(!is_array($tarif))$tarif=array();$m=$this->db->where('id',$id)->get('tagihan_master')->row_array();if(!$m||!$tarif)return model_response(false,'Data tarif tidak lengkap.');$this->db->trans_begin();foreach($tarif as $idTarget=>$nominal){$idTarget=(int)$idTarget;$nominal=nilai_nominal($nominal);if($nominal<0){$this->db->trans_rollback();return model_response(false,'Tarif tidak boleh negatif.');}$target=$this->db->where('id',$idTarget)->where('id_tagihan_master',$id)->get('tagihan_target_kelas')->row_array();if(!$target)continue;$maxPaid=(float)$this->db->select_max('nominal_dibayar','max')->where('id_tagihan_master',$id)->where('id_kelas_setting',$target['id_kelas_setting'])->get('tagihan_siswa')->row()->max;if($nominal<$maxPaid){$this->db->trans_rollback();return model_response(false,'Tarif kelas '.$target['nama_kelas'].' tidak boleh lebih kecil dari pembayaran yang sudah masuk.');}$before=$target;$this->db->where('id',$idTarget)->update('tagihan_target_kelas',array('nominal_kelas'=>$nominal,'tanggal'=>tanggal_sekarang(),'waktu'=>waktu_sekarang(),'id_user'=>app_user_id(),'nama_user'=>app_user_name()));$rows=$this->db->where('id_tagihan_master',$id)->where('id_kelas_setting',$target['id_kelas_setting'])->get('tagihan_siswa')->result_array();foreach($rows as $row){$special=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$row['id_siswa'])->where('jenis_keringanan','Tarif Khusus')->where('status','Aktif')->order_by('id','DESC')->get('tagihan_keringanan_siswa')->row_array();if($special)continue;$newSisa=max(0,$nominal-(float)$row['nominal_dibayar']);$status=$newSisa<=0?'Lunas':((float)$row['nominal_dibayar']>0?'Dibayar Sebagian':'Belum Dibayar');$this->db->where('id',$row['id'])->update('tagihan_siswa',array('nominal_awal'=>$nominal,'nominal_tagihan'=>$nominal,'sisa_tagihan'=>$newSisa,'status_pembayaran'=>$status,'tanggal_update'=>tanggal_sekarang(),'waktu_update'=>waktu_sekarang()));}tagihan_log_activity('Ubah Tarif Kelas','Tagihan','Ubah','tagihan_target_kelas',$idTarget,$m['kode_tagihan'],$target['nama_kelas'].' menjadi '.rupiah($nominal),$before,array('nominal_kelas'=>$nominal));}
    ...
    public function simpan_siswa(){
        $id=(int)$this->input->post('id_tagihan');$sid=(int)$this->input->post('id_siswa');$nominal=nilai_nominal($this->input->post('nominal_khusus'));$alasan=trim((string)$this->input->post('alasan',true));$m=$this->db->where('id',$id)->get('tagihan_master')->row_array();$s=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_siswa')->row_array();if(!$m||!$s||$alasan===''||$nominal<0)return model_response(false,'Siswa, nominal khusus, dan alasan wajib diisi.');$maxPaid=(float)$this->db->select_max('nominal_dibayar','max')->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_siswa')->row()->max;if($nominal<$maxPaid)return model_response(false,'Tarif khusus tidak boleh lebih kecil dari pembayaran yang sudah masuk.');
        ...
        tagihan_log_activity('Tarif Khusus Siswa','Tagihan','Ubah','tagihan_siswa',$sid,$m['kode_tagihan'],$s['nama_siswa'].' menjadi '.rupiah($nominal).' - '.$alasan,$s,array('nominal_khusus'=>$nominal));return tagihan_transaction_result('Tarif khusus siswa berhasil disimpan.');
    }
