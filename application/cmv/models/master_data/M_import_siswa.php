...
class M_import_siswa extends CI_Model
{
    ...
    public function riwayat(){return $this->db->order_by('id','DESC')->limit(20)->get('tagihan_import_siswa')->result_array();}
    ...
    public function preview(){
        ...
        for($i=1;$i<count($rows);$i++){
            ...
            $errors=array();$nis=$item['NIS'];$nisn=$item['NISN'];$nama=$item['NAMA_LENGKAP'];$jk=$item['JENIS_KELAMIN'];
            if($nis===''||$nisn===''||$nama===''||$jk==='')$errors[]='Kolom wajib belum lengkap';
            if(isset($seenNis[$nis]))$errors[]='NIS duplikat dalam file';if(isset($seenNisn[$nisn]))$errors[]='NISN duplikat dalam file';$seenNis[$nis]=true;$seenNisn[$nisn]=true;
            if($nis!==''&&$this->db->where('nis',$nis)->count_all_results('siswa'))$errors[]='NIS sudah ada di database';if($nisn!==''&&$this->db->where('nisn',$nisn)->count_all_results('siswa'))$errors[]='NISN sudah ada di database';
            if(!empty($item['NAMA_KELAS'])&&strcasecmp($item['NAMA_KELAS'],$kelas['nama_kelas'])!==0)$errors[]='Nama kelas tidak sama dengan kelas penempatan';
            $status=$errors?'Gagal':'Valid';if($errors)$invalid++;else$valid++;
    ...
    public function proses(){
        ...
        foreach($session['rows'] as $row){
            $idSiswa=0;$status=$row['status']==='Valid'?'Berhasil':'Gagal';$message=$row['pesan'];
            if($row['status']==='Valid'){
                $d=$row['data'];$jk=strtolower($d['JENIS_KELAMIN']);$jk=in_array($jk,array('l','laki-laki','laki laki'),true)?'Laki-laki':(in_array($jk,array('p','perempuan'),true)?'Perempuan':$d['JENIS_KELAMIN']);
                $siswa=array('id_daftar_siswa'=>'0','nisn'=>$d['NISN'],'nis'=>$d['NIS'],'nama_lengkap'=>$d['NAMA_LENGKAP'],'jk'=>$jk,'tempat_lahir'=>isset($d['TEMPAT_LAHIR'])?$d['TEMPAT_LAHIR']:'','tanggal_lahir'=>isset($d['TANGGAL_LAHIR'])?$d['TANGGAL_LAHIR']:'','tanggal_awal_masuk'=>isset($d['TANGGAL_AWAL_MASUK'])?$d['TANGGAL_AWAL_MASUK']:tanggal_sekarang(),'foto_siswa'=>'','status_pendaftaran'=>'Aktif','id_periode'=>(string)$session['id_periode'],'alamat_siswa'=>isset($d['ALAMAT'])?$d['ALAMAT']:'','nama_ayah'=>isset($d['NAMA_AYAH'])?$d['NAMA_AYAH']:'','pekerjaan_ayah'=>isset($d['PEKERJAAN_AYAH'])?$d['PEKERJAAN_AYAH']:'','telepon_ayah'=>isset($d['TELEPON_AYAH'])?$d['TELEPON_AYAH']:'','nama_ibu'=>isset($d['NAMA_IBU'])?$d['NAMA_IBU']:'','pekerjaan_ibu'=>isset($d['PEKERJAAN_IBU'])?$d['PEKERJAAN_IBU']:'','telepon_ibu'=>isset($d['TELEPON_IBU'])?$d['TELEPON_IBU']:'');
                $this->db->insert('siswa',$siswa);$idSiswa=$this->db->insert_id();$this->db->insert('kelas_siswa',array('id_kelas_setting'=>(string)$session['id_kelas_setting'],'id_siswa'=>(string)$idSiswa,'nama_siswa'=>$d['NAMA_LENGKAP'],'nisn'=>$d['NISN'],'jenis_kelamin'=>$jk,'status_aktif'=>'1'));$success++;
            }
