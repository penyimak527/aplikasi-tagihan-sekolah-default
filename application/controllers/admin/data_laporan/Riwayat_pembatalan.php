...
class Riwayat_pembatalan extends CI_Controller
{
    ...
    {
        $laporan = $this->model->report('pembatalan');
        ...
        $data = array(
            ...
            'laporan' => $laporan,
            'filter_laporan' => $this->model->print_filter('pembatalan'),
            'petugas' => isset($admin['nama']) && $admin['nama'] !== '' ? $admin['nama'] : (isset($admin['username']) ? $admin['username'] : 'Administrator'),
            'orientation' => 'landscape'
        );

        $this->load->view('admin/data_laporan/riwayat_pembatalan', $data);
    }
}
