<div class="card">
    ...
    <div class="card-body">
        ...
        <div id="hasil_siswa" class="crud-list"></div>
    </div>
</div>
...
$(document).on('click', '.btn-riwayat', function () {
    $.post('<?= base_url('tarif_khusus_siswa/riwayat') ?>', {id_tagihan: $('#tagihan').val(), id_siswa: $(this).data('siswa')}, function (rows) {
        var html = rows.length ? rows.map(function (row) {
            return '<tr><td>' + escapeHtml((row.tanggal || '-') + ' ' + (row.waktu || '')) + '</td><td class="text-end">' + formatRupiah(row.nominal_awal || 0) + '</td><td class="text-end">' + formatRupiah(row.nominal_setelah_keringanan || 0) + '</td><td>' + escapeHtml(row.alasan || '-') + '</td><td>' + escapeHtml(row.nama_user || '-') + '</td><td><span class="badge bg-' + (row.status === 'Aktif' ? 'success' : 'secondary') + '">' + escapeHtml(row.status) + '</span></td></tr>';
