...
function filterKelasTinggal(side) {
    ...
    $('#kelas_' + side).val('');
    if (side === 'asal') $('#data').html('<tr><td colspan="4"><div class="empty-state">Pilih kelas asal.</div></td></tr>');
}
...
function loadSiswaTinggal() {
    ...
    $.post('<?= base_url('tinggal_kelas/siswa') ?>', {id_kelas_setting: id}, function (rows) {
        ...
        $('#data').html(rows.map(function (row) {
            return '<tr><td><input type="checkbox" class="form-check-input pilih-siswa" value="' + row.id + '"></td><td>' + escapeHtml(row.nis || '-') + '</td><td><strong>' + escapeHtml(row.nama_lengkap) + '</strong><br><small class="text-muted">NISN ' + escapeHtml(row.nisn || '-') + '</small></td><td>' + escapeHtml(kelasAsal) + '</td></tr>';
        }).join(''));
    }, 'json').fail(ajaxError);
}
