...
function loadSiswaKelulusan() {
    ...
    $.post('<?= base_url('kelulusan/siswa') ?>', {id_kelas_setting: $('#kelas').val()}, function (rows) {
        ...
        $('#data').html(rows.map(function (row) {
            ...
            return '<tr><td><input type="checkbox" class="form-check-input pilih-siswa" value="' + row.id + '"></td><td><strong>' + escapeHtml(row.nama_lengkap) + '</strong><br><small class="text-muted">NIS ' + escapeHtml(row.nis || '-') + ' | NISN ' + escapeHtml(row.nisn || '-') + '</small></td><td>' + tagihan + '</td><td>' + escapeHtml(row.status_pendaftaran || '-') + '</td></tr>';
        }).join(''));
    }, 'json').fail(ajaxError);
}
