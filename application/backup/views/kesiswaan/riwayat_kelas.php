...
function cari() {
    ...
    $('#btn_cari').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Mencari');
    $.post('<?= base_url('riwayat_kelas/cari') ?>', { q: keyword }, function (rows) {
        var html = '';
        if (!rows.length) {
        ...
        rows.forEach(function (row) {
            html += '<div class="crud-list-item">' +
                '<div class="crud-content">' +
                    ...
                    '<div class="crud-meta">NIS: ' + escapeHtml(row.nis || '-') + ' | NISN: ' + escapeHtml(row.nisn || '-') + '</div>' +
                    '<div class="crud-status mt-1"><span class="badge bg-primary-subtle text-primary">' + escapeHtml(row.status_pendaftaran || '-') + '</span></div>' +
                '</div>' +
                '<div class="crud-actions">' +
                    '<button type="button" class="btn btn-primary" data-student-id="' + Number(row.id) + '"><i class="ri-check-line me-1"></i>Pilih</button>' +
                '</div>' +
            '</div>';
        });
        $('#hasil_cari').html(html);
    }, 'json').fail(ajaxError).always(function () {
        $('#btn_cari').prop('disabled', false).html('<i class="ri-search-line me-1"></i>Cari');
    });
}

function loadDetail(id) {
    $.post('<?= base_url('riwayat_kelas/result') ?>', { id_siswa: id }, function (response) {
...
function simpanKoreksi() {
    ...
    Swal.fire({
        ...
        var button = $('#btn_simpan_koreksi');
        button.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Menyimpan');
        $.post('<?= base_url('riwayat_kelas/koreksi') ?>', {
            id_siswa: selectedStudentId,
            id_kelas_tujuan: targetClassId,
            alasan: reason
        }, function (response) {
            if (response.result !== 'true') {
                Swal.fire('Gagal', response.message || 'Koreksi penempatan gagal.', 'error');
                return;
            }
            modalKoreksi.hide();
            Swal.fire(response.warning ? 'Berhasil dengan Peringatan' : 'Berhasil', response.message, response.warning ? 'warning' : 'success');
            loadDetail(selectedStudentId);
        }, 'json').fail(ajaxError).always(function () {
            button.prop('disabled', false).html('<i class="ri-save-line me-1"></i>Simpan Koreksi');
        });
    });
}
</script>
