...
class M_kenaikan_kelas extends CI_Model
{
    ...
    public function preview($jenis){
        ...
        if(!$asal||!$tujuan||!$ids)return model_response(false,'Kelas asal, kelas tujuan, dan siswa wajib dipilih.');
        if((int)$asal['id_periode']===(int)$tujuan['id_periode'])return model_response(false,'Tahun ajaran tujuan harus berbeda dari tahun ajaran asal.');
        $valid=0;$skip=0;
        foreach($ids as $sid){
            $sid=(int)$sid;
        ...
        return model_response(true,'Preview siap.',array('jenis'=>$jenis,'asal'=>$asal,'tujuan'=>$tujuan,'dipilih'=>count($ids),'valid'=>$valid,'dilewati'=>$skip));
    }
    public function proses($jenis){
        $asalId=(int)$this->input->post('id_kelas_asal');$tujuanId=(int)$this->input->post('id_kelas_tujuan');$ids=$this->input->post('id_siswa');$alasan=trim((string)$this->input->post('alasan',true));if(!is_array($ids))$ids=array();$asal=$this->db->where('id',$asalId)->get('kelas_setting')->row_array();$tujuan=$this->db->where('id',$tujuanId)->get('kelas_setting')->row_array();if(!$asal||!$tujuan||!$ids)return model_response(false,'Kelas asal, kelas tujuan, dan siswa wajib dipilih.');if((int)$asal['id_periode']===(int)$tujuan['id_periode'])return model_response(false,'Tahun ajaran tujuan harus berbeda dari tahun ajaran asal.');
        ...
        tagihan_log_activity($jenis,'Kesiswaan','Tambah','kelas_siswa',$tujuanId,$tujuan['nama_kelas'],$jenis.' '.$success.' siswa; dilewati '.$skip,null,array('asal'=>$asalId,'tujuan'=>$tujuanId,'siswa'=>$ids));return tagihan_transaction_result($success.' siswa berhasil diproses'.($skip?' dan '.$skip.' dilewati.':'.'));
    }
}
