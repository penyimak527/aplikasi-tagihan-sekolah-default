...
class M_pembayaran extends MY_Model
{
    ...
    public function siswa_by_id($id){
        ...
        return $row?array('result'=>'true','siswa'=>$row):$this->response(false,'Siswa tidak ditemukan.');
    }
    public function tagihan_siswa()
    {
        ...
        if (!$siswa) {
            return $this->response(false, 'Siswa tidak ditemukan.');
        }
    ...
    public function simpan(){
        $token=trim((string)$this->input->post('token',true));$active=(string)$this->session->userdata('token_pembayaran_aktif');if($token===''||!hash_equals($active,$token))return $this->response(false,'Token transaksi tidak valid atau transaksi sudah diproses. Muat ulang halaman.');
        ...
        if(!$idSiswa||!$idMetode||!is_array($items)||!count($items))return $this->response(false,'Siswa, metode, dan minimal satu tagihan wajib dipilih.');
        if(!preg_match('/^\d{2}-\d{2}-\d{4}$/',$tanggal))return $this->response(false,'Format tanggal pembayaran harus dd-mm-yyyy.');
        $siswa=$this->db->query("SELECT s.*,ks.id_kelas_setting,k.id_kelas,k.nama_kelas,k.id_periode,ta.periode FROM siswa s LEFT JOIN kelas_siswa ks ON CAST(ks.id_siswa AS UNSIGNED)=s.id AND ks.status_aktif='1' LEFT JOIN kelas_setting k ON k.id=CAST(ks.id_kelas_setting AS UNSIGNED) LEFT JOIN master_tahun_ajaran ta ON ta.id=CAST(k.id_periode AS UNSIGNED) WHERE s.id=? LIMIT 1",array($idSiswa))->row_array();$metode=$this->db->where('id',$idMetode)->where('status','Aktif')->get('tagihan_metode_pembayaran')->row_array();if(!$siswa||!$metode)return $this->response(false,'Data siswa atau metode pembayaran tidak ditemukan.');
        $normalized=array();$total=0;$seen=array();foreach($items as $item){$id=(int)($item['id_tagihan_siswa']??0);$bayar=nilai_nominal($item['nominal_bayar']??0);if(!$id||$bayar<=0||isset($seen[$id]))return $this->response(false,'Keranjang mengandung data tidak valid atau tagihan ganda.');$seen[$id]=1;$row=$this->db->where('id',$id)->where('id_siswa',$idSiswa)->where('status_tagihan','Aktif')->get('tagihan_siswa')->row_array();if(!$row||in_array($row['status_pembayaran'],array('Lunas','Dibebaskan','Dibatalkan'),true))return $this->response(false,'Salah satu tagihan tidak lagi dapat dibayar. Silakan muat ulang daftar.');if($bayar-(float)$row['sisa_tagihan']>0.001)return $this->response(false,'Nominal bayar '.$row['nama_tagihan'].' melebihi sisa tagihan.');$normalized[]=array('row'=>$row,'bayar'=>$bayar);$total+=$bayar;}
        if($total<=0)return $this->response(false,'Total pembayaran harus lebih dari nol.');if($metode['butuh_uang_diterima']==='Ya'&&$uang+0.001<$total)return $this->response(false,'Uang diterima kurang dari total pembayaran.');if($metode['butuh_uang_diterima']!=='Ya')$uang=$total;$kembali=max(0,$uang-$total);
        $this->db->trans_begin();$no=$this->next_code('BYR','tagihan_pembayaran','no_transaksi');$header=array('no_transaksi'=>$no,'tanggal_transaksi'=>$tanggal,'waktu_transaksi'=>waktu_sekarang(),'id_siswa'=>$idSiswa,'nis'=>$siswa['nis'],'nisn'=>$siswa['nisn'],'nama_siswa'=>$siswa['nama_lengkap'],'id_kelas_setting'=>(int)($siswa['id_kelas_setting']??0),'id_kelas'=>(int)($siswa['id_kelas']??0),'nama_kelas'=>$siswa['nama_kelas']??'-','id_periode'=>(int)($siswa['id_periode']??0),'periode'=>$siswa['periode']??'-','total_tagihan_dipilih'=>$total,'total_potongan'=>0,'total_pembayaran'=>$total,'id_metode_pembayaran'=>$idMetode,'nama_metode_pembayaran'=>$metode['nama_metode'],'uang_diterima'=>$uang,'kembalian'=>$kembali,'referensi_pembayaran'=>$referensi,'status_transaksi'=>'Aktif','status_cetak'=>'Belum','jumlah_cetak'=>0,'status_kirim_whatsapp'=>'Belum','keterangan'=>$catatan,'id_user'=>app_user_id(),'nama_user'=>app_user_name());$this->db->insert('tagihan_pembayaran',$header);$idBayar=(int)$this->db->insert_id();
        ...
        $this->log_activity('Terima Pembayaran','Transaksi','Tambah','tagihan_pembayaran',$idBayar,$no,'Pembayaran '.$siswa['nama_lengkap'].' sebesar '.rupiah($total),null,$header);
        if($this->db->trans_status()===FALSE){$this->db->trans_rollback();return $this->response(false,'Penyimpanan pembayaran gagal. Seluruh perubahan dibatalkan.');}$this->db->trans_commit();$this->session->unset_userdata('token_pembayaran_aktif');return $this->response(true,'Pembayaran berhasil disimpan.',array('id_pembayaran'=>$idBayar,'no_transaksi'=>$no,'total'=>$total,'uang_diterima'=>$uang,'kembalian'=>$kembali));
    }
    public function detail($id){
        $header=$this->db->where('id',$id)->get('tagihan_pembayaran')->row_array();if(!$header)return $this->response(false,'Transaksi tidak ditemukan.');$detail=$this->db->where('id_pembayaran',$id)->order_by('id')->get('tagihan_pembayaran_detail')->result_array();$siswa=$this->db->where('id',$header['id_siswa'])->get('siswa')->row_array();$cancel=$this->db->where('id_pembayaran',$id)->get('tagihan_pembatalan_transaksi')->row_array();return array('result'=>'true','header'=>$header,'detail'=>$detail,'siswa'=>$siswa,'pembatalan'=>$cancel);
    }
