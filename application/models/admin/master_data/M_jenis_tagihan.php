...
class M_jenis_tagihan extends CI_Model
{
    public function result()
    {
        ...
        if ($search !== '') $this->db->group_start()->like('nama_jenis', $search)->or_like('kode_jenis', $search)->group_end();
        if ($tipe !== '') $this->db->where('tipe_default', $tipe);
    ...
    public function simpan()
    {
        ...
        $tipe = $this->input->post('tipe_default', true);
        ...
        if ($nama === '' || !in_array($tipe, array('Bulanan', 'Langsung', 'Tahunan'), true)) return $this->model_response(false, 'Nama dan tipe tagihan wajib diisi.');
        if ($kode === '') $kode = 'JNS-' . strtoupper(substr(preg_replace('/[^A-Za-z0-9]/', '', $nama), 0, 12));
        $exists = $this->db->where('nama_jenis', $nama)->where('status', 'Aktif')->where('id !=', $id)->count_all_results('tagihan_jenis');
        if ($exists) return $this->model_response(false, 'Nama jenis tagihan aktif sudah digunakan.');
        $before = $id ? $this->db->where('id', $id)->get('tagihan_jenis')->row_array() : null;
        $data = array_merge(array('kode_jenis' => $kode, 'nama_jenis' => $nama, 'tipe_default' => $tipe, 'dianggap_tunggakan' => $tunggakan, 'status' => $status, 'keterangan' => $ket), $this->tagihan_audit_fields());
        $this->db->trans_begin();
        if ($id) {
        ...
        $this->tagihan_log_activity($before ? 'Ubah Jenis Tagihan' : 'Tambah Jenis Tagihan', 'Master Data', $before ? 'Ubah' : 'Tambah', 'tagihan_jenis', $id, $kode, 'Pengelolaan jenis tagihan', $before, $data);
        return $this->tagihan_transaction_result('Jenis tagihan berhasil disimpan.');
    }
    public function ubah_status()
    {
        $id = (int)$this->input->post('id');
        $row = $this->db->where('id', $id)->get('tagihan_jenis')->row_array();
        if (!$row) return $this->model_response(false, 'Data tidak ditemukan.');
        $status = $row['status'] === 'Aktif' ? 'Nonaktif' : 'Aktif';
        $this->db->trans_begin();
        $this->db->where('id', $id)->update('tagihan_jenis', array('status' => $status, 'tanggal' => $this->tanggal_sekarang(), 'waktu' => $this->waktu_sekarang()));
        $this->tagihan_log_activity('Ubah Status Jenis Tagihan', 'Master Data', 'Ubah', 'tagihan_jenis', $id, $row['kode_jenis'], 'Status menjadi ' . $status, $row, array('status' => $status));
        return $this->tagihan_transaction_result('Status jenis tagihan berhasil diubah.');
    }
