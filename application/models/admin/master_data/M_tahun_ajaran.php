...
class M_tahun_ajaran extends CI_Model
{
    ...
    public function simpan(){
        ...
        if(!preg_match('/^(\d{4})\/(\d{4})$/',$periode,$m)||((int)$m[2]!==((int)$m[1]+1)))return model_response(false,'Format tahun ajaran harus YYYY/YYYY dan tahun kedua satu tahun setelah tahun pertama.');
        $exists=$this->db->where('periode',$periode)->where('id !=',$id)->count_all_results('master_tahun_ajaran');
        if($exists)return model_response(false,'Tahun ajaran tersebut sudah tersedia.');
        ...
        tagihan_log_activity($before?'Ubah Tahun Ajaran':'Tambah Tahun Ajaran','Master Data',$before?'Ubah':'Tambah','master_tahun_ajaran',$id,$periode,'Pengelolaan tahun ajaran',$before,$data);
        return tagihan_transaction_result('Tahun ajaran berhasil disimpan.');
    }
    public function aktifkan(){
        $id=(int)$this->input->post('id');$row=$this->db->where('id',$id)->get('master_tahun_ajaran')->row_array();if(!$row)return model_response(false,'Tahun ajaran tidak ditemukan.');
        ...
        tagihan_log_activity('Aktifkan Tahun Ajaran','Master Data','Ubah','master_tahun_ajaran',$id,$row['periode'],'Menetapkan tahun ajaran aktif',$row,array('status'=>'Aktif'));
        return tagihan_transaction_result('Tahun ajaran berhasil diaktifkan.');
    }
    public function hapus(){
        $id=(int)$this->input->post('id');$row=$this->db->where('id',$id)->get('master_tahun_ajaran')->row_array();if(!$row)return model_response(false,'Data tidak ditemukan.');
        if($row['status']==='Aktif')return model_response(false,'Tahun ajaran aktif tidak dapat dihapus.');
        if($this->db->where('id_periode',(string)$id)->count_all_results('kelas_setting')||$this->db->where('id_periode',$id)->count_all_results('tagihan_master'))return model_response(false,'Tahun ajaran sudah digunakan dan tidak dapat dihapus.');
        $this->db->trans_begin();$this->db->where('id',$id)->delete('master_tahun_ajaran');tagihan_log_activity('Hapus Tahun Ajaran','Master Data','Batal','master_tahun_ajaran',$id,$row['periode'],'Menghapus tahun ajaran yang belum digunakan',$row,null);
        return tagihan_transaction_result('Tahun ajaran berhasil dihapus.');
    }
}
