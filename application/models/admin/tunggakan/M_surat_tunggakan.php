...
class M_surat_tunggakan extends CI_Model
{
    ...
    public function simpan()
    {
        ...
        $bulan = (int)$this->input->post('batas_bulan');
        $tahun = (int)$this->input->post('batas_tahun');
        if (!$idSiswa || !is_array($ids) || !count($ids)) return model_response(false, 'Siswa dan minimal satu tagihan wajib dipilih.');
        if ($tanggal === '' || $nama === '' || $jabatan === '') return model_response(false, 'Tanggal dan penandatangan wajib diisi.');
        ...
        tagihan_log_activity('Buat Surat Tunggakan', 'Tunggakan', 'Tambah', 'tagihan_surat_tunggakan', $idSurat, $no, 'Surat ' . $s['nama_lengkap'] . ' sebesar ' . rupiah($total), null, $head);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return model_response(false, 'Surat gagal disimpan.');
        }
        $this->db->trans_commit();
        return model_response(true, 'Surat tunggakan berhasil disimpan.', array('id' => $idSurat, 'no_surat' => $no));
    }
    ...
    public function detail($id)
    {
        ...
        if (!$h) return model_response(false, 'Surat tidak ditemukan.');
    ...
    private function template($tpl, $h, $s, $namaPenerima = '')
    {
        ...
        $map = array(
            ...
            '{total_bayar}' => '',
            '{total_tunggakan}' => rupiah($h['total_tunggakan']),
    ...
    public function siapkan_whatsapp()
    {
        ...
        $no = bersihkan_nomor_wa((string)$this->input->post('nomor', true));
        ...
        if ($no === '') return model_response(false, 'Nomor WhatsApp wajib diisi.');
        ...
        if ($nama === '') {
            return model_response(false, 'Nama penerima WhatsApp wajib diisi.');
        }
        ...
        tagihan_log_activity('Kirim Surat Tunggakan WhatsApp', 'Tunggakan', 'Kirim', 'tagihan_surat_tunggakan', $id, $d['header']['no_surat'], 'Surat disiapkan ke ' . $no);
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return model_response(false, 'Gagal menyiapkan WhatsApp.');
        }
        $this->db->trans_commit();
        return model_response(true, 'WhatsApp berhasil disiapkan.', array('url' => 'https://wa.me/' . $no . '?text=' . rawurlencode($pesan), 'pesan' => $pesan));
    }
}
