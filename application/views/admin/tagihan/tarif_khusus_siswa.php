...
function cariSiswa() {
    ...
    $.post('<?= base_url('admin/tagihan/tarif_khusus_siswa/cari_siswa') ?>', {id_tagihan: id, q: $('#q_siswa').val()}, function (rows) {
        if (!rows.length) {
...
$(document).on('click', '.btn-normal', function () {
    ...
    confirmAction('Kembalikan ke tarif normal?', 'Tarif khusus aktif akan dibatalkan. Pembayaran yang sudah masuk tetap dipertahankan.', function () {
        $.post('<?= base_url('admin/tagihan/tarif_khusus_siswa/kembalikan_normal') ?>', {id_tagihan: $('#tagihan').val(), id_siswa: idSiswa, alasan: 'Dikembalikan ke tarif normal'}, function (response) {
            var ok = response.result === 'true';
            Swal.fire(ok ? 'Berhasil' : 'Gagal', response.message, ok ? 'success' : 'error');
...
$(document).on('click', '.btn-riwayat', function () {
    $.post('<?= base_url('admin/tagihan/tarif_khusus_siswa/riwayat') ?>', {id_tagihan: $('#tagihan').val(), id_siswa: $(this).data('siswa')}, function (rows) {
