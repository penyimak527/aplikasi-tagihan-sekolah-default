...
<script>
    ...
    function loadData() {
        $.post('<?= base_url('admin/tagihan/daftar_tagihan/result') ?>', {
    ...
    function detail(id) {
        $.post('<?= base_url('admin/tagihan/daftar_tagihan/detail') ?>', {
            id: id
        }, function(r) {
    ...
    function terbitkan(id) {
        confirmAction('Terbitkan draft tagihan?', 'Tagihan siswa akan dibuat berdasarkan target dan tarif tersimpan.', function() {
            $.post('<?= base_url('admin/tagihan/daftar_tagihan/terbitkan') ?>', {
                id: id
            }, function(r) {
    ...
    function batalkan(id) {
        Swal.fire({
            ...
            if (res.isConfirmed) $.post('<?= base_url('admin/tagihan/daftar_tagihan/batalkan_sisa') ?>', {
                id: id,
                alasan: res.value
            }, function(r) {
                Swal.fire(r.result === 'true' ? 'Berhasil' : 'Gagal', r.message, r.result === 'true' ? 'success' : 'error');
