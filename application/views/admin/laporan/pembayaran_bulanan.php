            ...
            <div class="col-md-6 col-xl-2">
                ...
                <select id="filter_periode" class="form-select">
                    ...
                    <?php foreach ($periode as $row): ?>
                        <option value="<?= (int) $row['id'] ?>" <?= $row['status'] === 'Aktif' ? 'selected' : '' ?>><?= html_escape($row['periode']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="col-md-6 col-xl-2">
                ...
                <select id="filter_bulan" class="form-select">
                    <?php for ($month = 1; $month <= 12; $month++): ?>
                        <option value="<?= $month ?>" <?= $month === (int) date('n') ? 'selected' : '' ?>><?= nama_bulan($month) ?></option>
                    <?php endfor; ?>
                </select>
            </div>
            <div class="col-md-6 col-xl-2">
                <label class="form-label" for="filter_tahun">Tahun Kalender</label>
                <input id="filter_tahun" type="number" class="form-control" value="<?= date('Y') ?>">
            </div>
            <div class="col-md-6 col-xl-2">
...
$(document).ready(function () {
    ...
    function loadReport() {
        ...
        $.ajax({
            url: '<?= base_url('admin/laporan/laporan/result/bulanan') ?>',
            type: 'POST',
            data: reportData(),
            dataType: 'JSON',
    ...
    $('#cetak').on('click', function () {
        var url = '<?= base_url('admin/data_laporan/pembayaran_bulanan') ?>?' + new URLSearchParams(reportData()).toString();
        window.open(url, '_blank');
    });
    $('#export').on('click', function () {
        this.href = '<?= base_url('admin/laporan/laporan/export/bulanan') ?>?' + new URLSearchParams(reportData()).toString();
    });
