...
<div class="row g-3 mb-3" id="dashboard_metrics">
    <?php foreach ($metricCards as $card): ?>
        <div class="col-12 col-md-6 col-xxl-3">
            <div class="card dashboard-metric-card <?= isset($card['url']) ? 'is-link' : '' ?>"
                 <?= isset($card['url']) ? 'data-url="' . html_escape($card['url']) . '"' : '' ?>>
                <div class="card-body">
                    ...
                    <div class="text-end min-w-0">
                        ...
                        <div class="summary-value" id="sum_<?= $card['key'] ?>" data-money="<?= $card['money'] ? '1' : '0' ?>">-</div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0 align-middle">
                        ...
                        <tbody id="transaksi_terbaru">
                            <tr><td colspan="7"><div class="empty-state py-4">Memuat data...</div></td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0 align-middle">
                        ...
                        <tbody id="tunggakan_prioritas">
                            <tr><td colspan="5"><div class="empty-state py-4">Memuat data...</div></td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
...
function showDashboardError() {
    ...
    $('#transaksi_terbaru').html('<tr><td colspan="7"><div class="empty-state py-4">Data gagal dimuat.</div></td></tr>');
    $('#tunggakan_prioritas').html('<tr><td colspan="5"><div class="empty-state py-4">Data gagal dimuat.</div></td></tr>');
    if (dashboardChart) {
...
function renderTransaksiTerbaru(rows) {
    if (!rows.length) {
        $('#transaksi_terbaru').html(
            '<tr><td colspan="7"><div class="empty-state py-4">' +
...
function renderTunggakanPrioritas(rows) {
    if (!rows.length) {
        $('#tunggakan_prioritas').html('<tr><td colspan="5"><div class="empty-state py-4">Tidak ada tunggakan prioritas.</div></td></tr>');
        return;
    }
