...
class M_kelulusan extends MY_Model
{
    ...
    public function preview(){
        ...
        if(!$kelas||!$ids)return $this->response(false,'Kelas dan siswa wajib dipilih.');
        $tunggakan=0;$lunas=0;
        foreach($ids as $sid){
        ...
        return $this->response(true,'Preview siap.',array('kelas'=>$kelas,'jumlah'=>count($ids),'masih_tunggakan'=>$tunggakan,'lunas'=>$lunas));
    }
    public function proses(){
        $idKelas=(int)$this->input->post('id_kelas_setting');$ids=$this->input->post('id_siswa');$tanggal=trim((string)$this->input->post('tanggal_lulus',true));$tahun=trim((string)$this->input->post('tahun_kelulusan',true));if(!is_array($ids))$ids=array();$kelas=$this->db->where('id',$idKelas)->get('kelas_setting')->row_array();if(!$kelas||!$ids||$tanggal===''||$tahun==='')return $this->response(false,'Kelas, siswa, tanggal lulus, dan tahun kelulusan wajib diisi.');$periode=$this->db->where('id',(int)$kelas['id_periode'])->get('master_tahun_ajaran')->row_array();
        ...
        $this->log_activity('Kelulusan Siswa','Kesiswaan','Ubah','siswa',$idKelas,$kelas['nama_kelas'],'Meluluskan '.$success.' siswa tahun '.$tahun,null,array('siswa'=>$ids));return $this->transaction_result($success.' siswa berhasil diproses lulus.');
    }
}
