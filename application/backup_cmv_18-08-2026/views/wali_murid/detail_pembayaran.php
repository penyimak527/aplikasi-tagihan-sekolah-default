    ...
    <div class="card-body">
        <?php foreach ($detail as $row): ?>
            <div class="d-flex justify-content-between border-bottom py-2 gap-3">
                ...
                </div><strong><?= $format_rupiah($row['nominal_bayar']) ?></strong>
            </div>
        <?php endforeach; ?>
        <div class="d-flex justify-content-between pt-3"><strong>TOTAL</strong><strong
                class="fs-5"><?= $format_rupiah($header['total_pembayaran']) ?></strong></div>
        <div class="d-flex justify-content-between mt-2"><span>Uang
                Diterima</span><span><?= $format_rupiah($header['uang_diterima']) ?></span></div>
        <div class="d-flex justify-content-between">
            <span>Kembalian</span><span><?= $format_rupiah($header['kembalian']) ?></span></div>
    </div>
</div>