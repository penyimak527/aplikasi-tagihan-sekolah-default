    ...
    <div class="card-body">
        <div class="row g-2 align-items-end mb-0">
            ...
            <div class="col-md-2">
                ...
                <select class="form-select" data-width="100%" name="tahun" id="tahun">
                    <?php
                    $now = date('Y');
                    for ($a = 2025; $a <= $now; $a++) {
                    ?>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody id="data">
                    <tr>
                        <td colspan="4">
                            <div class="empty-state">Pilih tahun ajaran dan kelas akhir.</div>
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    ...
    function loadSiswaKelulusan() {
        ...
        $.post('<?= base_url('admin/kesiswaan/kelulusan/siswa') ?>', {
    ...
    function dataKelulusan() {
        return {
            id_kelas_setting: $('#kelas').val(),
            id_siswa: $('.pilih-siswa:checked').map(function() {
                return this.value;
            }).get(),
    ...
    function previewKelulusan() {
        ...
        $.post('<?= base_url('admin/kesiswaan/kelulusan/preview') ?>', data, function(response) {
    ...
    function prosesKelulusan() {
        ...
        confirmAction('Proses kelulusan ' + data.id_siswa.length + ' siswa?', 'Siswa tidak ikut tagihan baru. Tagihan dan pembayaran lama tetap tersimpan.', function() {
            ...
            $.post('<?= base_url('admin/kesiswaan/kelulusan/proses') ?>', data, function(response) {
