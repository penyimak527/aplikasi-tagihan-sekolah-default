...
class M_pembayaran extends CI_Model
{
    ...
    public function tagihan_siswa()
    {
        ...
        if (!$siswa) {
            return $this->model_response(false, 'Siswa tidak ditemukan.');
        }
    ...
    public function simpan(){
        $token=trim((string)$this->input->post('token',true));$active=(string)$this->session->userdata('token_pembayaran_aktif');if($token===''||!hash_equals($active,$token))return $this->model_response(false,'Token transaksi tidak valid atau transaksi sudah diproses. Muat ulang halaman.');
        $idSiswa=(int)$this->input->post('id_siswa');$idMetode=(int)$this->input->post('id_metode');$tanggal=trim((string)$this->input->post('tanggal',true));$uang=$this->nilai_nominal($this->input->post('uang_diterima'));$referensi=trim((string)$this->input->post('referensi',true));$catatan=trim((string)$this->input->post('catatan',true));$items=json_decode((string)$this->input->post('items'),true);
        if(!$idSiswa||!$idMetode||!is_array($items)||!count($items))return $this->model_response(false,'Siswa, metode, dan minimal satu tagihan wajib dipilih.');
        if(!preg_match('/^\d{2}-\d{2}-\d{4}$/',$tanggal))return $this->model_response(false,'Format tanggal pembayaran harus dd-mm-yyyy.');
        $siswa=$this->db->query("SELECT s.*,ks.id_kelas_setting,k.id_kelas,k.nama_kelas,k.id_periode,ta.periode FROM siswa s LEFT JOIN kelas_siswa ks ON CAST(ks.id_siswa AS UNSIGNED)=s.id AND ks.status_aktif='1' LEFT JOIN kelas_setting k ON k.id=CAST(ks.id_kelas_setting AS UNSIGNED) LEFT JOIN master_tahun_ajaran ta ON ta.id=CAST(k.id_periode AS UNSIGNED) WHERE s.id=? LIMIT 1",array($idSiswa))->row_array();$metode=$this->db->where('id',$idMetode)->where('status','Aktif')->get('tagihan_metode_pembayaran')->row_array();if(!$siswa||!$metode)return $this->model_response(false,'Data siswa atau metode pembayaran tidak ditemukan.');
        $normalized=array();$total=0;$seen=array();foreach($items as $item){$id=(int)($item['id_tagihan_siswa']??0);$bayar=$this->nilai_nominal($item['nominal_bayar']??0);if(!$id||$bayar<=0||isset($seen[$id]))return $this->model_response(false,'Keranjang mengandung data tidak valid atau tagihan ganda.');$seen[$id]=1;$row=$this->db->where('id',$id)->where('id_siswa',$idSiswa)->where('status_tagihan','Aktif')->get('tagihan_siswa')->row_array();if(!$row||in_array($row['status_pembayaran'],array('Lunas','Dibebaskan','Dibatalkan'),true))return $this->model_response(false,'Salah satu tagihan tidak lagi dapat dibayar. Silakan muat ulang daftar.');if($bayar-(float)$row['sisa_tagihan']>0.001)return $this->model_response(false,'Nominal bayar '.$row['nama_tagihan'].' melebihi sisa tagihan.');$normalized[]=array('row'=>$row,'bayar'=>$bayar);$total+=$bayar;}
        if($total<=0)return $this->model_response(false,'Total pembayaran harus lebih dari nol.');if($metode['butuh_uang_diterima']==='Ya'&&$uang+0.001<$total)return $this->model_response(false,'Uang diterima kurang dari total pembayaran.');if($metode['butuh_uang_diterima']!=='Ya')$uang=$total;$kembali=max(0,$uang-$total);
        $this->db->trans_begin();$no=$this->tagihan_next_code('BYR','tagihan_pembayaran','no_transaksi');$header=array('no_transaksi'=>$no,'tanggal_transaksi'=>$tanggal,'waktu_transaksi'=>$this->waktu_sekarang(),'id_siswa'=>$idSiswa,'nis'=>$siswa['nis'],'nisn'=>$siswa['nisn'],'nama_siswa'=>$siswa['nama_lengkap'],'id_kelas_setting'=>(int)($siswa['id_kelas_setting']??0),'id_kelas'=>(int)($siswa['id_kelas']??0),'nama_kelas'=>$siswa['nama_kelas']??'-','id_periode'=>(int)($siswa['id_periode']??0),'periode'=>$siswa['periode']??'-','total_tagihan_dipilih'=>$total,'total_potongan'=>0,'total_pembayaran'=>$total,'id_metode_pembayaran'=>$idMetode,'nama_metode_pembayaran'=>$metode['nama_metode'],'uang_diterima'=>$uang,'kembalian'=>$kembali,'referensi_pembayaran'=>$referensi,'status_transaksi'=>'Aktif','status_cetak'=>'Belum','jumlah_cetak'=>0,'status_kirim_whatsapp'=>'Belum','keterangan'=>$catatan,'id_user'=>$this->app_user_id(),'nama_user'=>$this->app_user_name());$this->db->insert('tagihan_pembayaran',$header);$idBayar=(int)$this->db->insert_id();
        foreach($normalized as $x){$r=$x['row'];$bayar=$x['bayar'];$paidBefore=(float)$r['nominal_dibayar'];$sisaBefore=(float)$r['sisa_tagihan'];$paidAfter=$paidBefore+$bayar;$sisaAfter=max(0,$sisaBefore-$bayar);$status=$this->payment_status($paidAfter,(float)$r['nominal_tagihan']);$this->db->insert('tagihan_pembayaran_detail',array('id_pembayaran'=>$idBayar,'no_transaksi'=>$no,'id_tagihan_siswa'=>$r['id'],'no_tagihan'=>$r['no_tagihan'],'id_tagihan_master'=>$r['id_tagihan_master'],'nama_tagihan'=>$r['nama_tagihan'],'tipe_tagihan'=>$r['tipe_tagihan'],'bulan'=>$r['bulan'],'nama_bulan'=>$r['nama_bulan'],'tahun'=>$r['tahun'],'nominal_tagihan'=>$r['nominal_tagihan'],'nominal_sudah_dibayar_sebelum'=>$paidBefore,'sisa_sebelum'=>$sisaBefore,'nominal_bayar'=>$bayar,'sisa_setelah'=>$sisaAfter,'status_setelah'=>$status,'status_detail'=>'Aktif','tanggal'=>$tanggal,'waktu'=>$this->waktu_sekarang()));$this->db->where('id',$r['id'])->update('tagihan_siswa',array('nominal_dibayar'=>$paidAfter,'sisa_tagihan'=>$sisaAfter,'status_pembayaran'=>$status,'tanggal_update'=>$this->tanggal_sekarang(),'waktu_update'=>$this->waktu_sekarang()));}
        $this->tagihan_log_activity('Terima Pembayaran','Transaksi','Tambah','tagihan_pembayaran',$idBayar,$no,'Pembayaran '.$siswa['nama_lengkap'].' sebesar '.$this->rupiah($total),null,$header);
        if($this->db->trans_status()===FALSE){$this->db->trans_rollback();return $this->model_response(false,'Penyimpanan pembayaran gagal. Seluruh perubahan dibatalkan.');}$this->db->trans_commit();$this->session->unset_userdata('token_pembayaran_aktif');return $this->model_response(true,'Pembayaran berhasil disimpan.',array('id_pembayaran'=>$idBayar,'no_transaksi'=>$no,'total'=>$total,'uang_diterima'=>$uang,'kembalian'=>$kembali));
    }
    public function detail($id){
        $header=$this->db->where('id',$id)->get('tagihan_pembayaran')->row_array();if(!$header)return $this->model_response(false,'Transaksi tidak ditemukan.');$detail=$this->db->where('id_pembayaran',$id)->order_by('id')->get('tagihan_pembayaran_detail')->result_array();$siswa=$this->db->where('id',$header['id_siswa'])->get('siswa')->row_array();$cancel=$this->db->where('id_pembayaran',$id)->get('tagihan_pembatalan_transaksi')->row_array();return array('result'=>'true','header'=>$header,'detail'=>$detail,'siswa'=>$siswa,'pembatalan'=>$cancel);
    }
    ...
    private function replace_template($tpl, $header, $siswa, $namaWali = '')
    {
        ...
        $map = array(
            ...
            '{total_bayar}' => $this->rupiah($header['total_pembayaran']),
            '{total_tunggakan}' => '',
    ...
    public function preview_whatsapp()
    {
        ...
        if ($data['header']['status_transaksi'] !== 'Aktif') {
            return $this->model_response(false, 'Bukti transaksi dibatalkan tidak dapat dikirim.');
        }
        ...
        return $this->model_response(true, 'Template WhatsApp berhasil dimuat.', array(
    ...
    public function siapkan_whatsapp()
    {
        ...
        $nomor = $this->bersihkan_nomor_wa((string) $this->input->post('nomor', true));
        $nama = trim((string) $this->input->post('nama_penerima', true));
        ...
        if ($nomor === '') {
            return $this->model_response(false, 'Nomor WhatsApp wajib diisi.');
        }
        ...
        return $this->model_response(true, 'Tautan WhatsApp berhasil disiapkan.', array(
    ...
    public function cek_baris_kartu()
    {
        ...
        if (!$format) {
            return $this->model_response(false, 'Format kartu tidak ditemukan atau tidak aktif.');
        }

        $config = $this->kartu_config($format);
        if ($baris < 1 || $baris > $config['jumlah_baris']) {
            return $this->model_response(false, 'Nomor baris berada di luar jumlah baris pada format kartu.');
        }
        ...
        return $this->model_response(true, '', array(
    ...
    public function catat_cetak_kartu()
    {
        ...
        if ($data['header']['status_transaksi'] !== 'Aktif') {
            return $this->model_response(false, 'Transaksi dibatalkan tidak dapat dicetak ke kartu.');
        }
        ...
        return $this->tagihan_transaction_result($message);
    }
