...
class M_tarif_khusus_siswa extends CI_Model
{
    ...
    public function simpan()
    {
        ...
        $nominal = $this->nilai_nominal($this->input->post('nominal_khusus'));
        ...
        if (!$master || !$sid || $alasan === '' || $nominal < 0) {
            return $this->model_response(false, 'Siswa, nominal khusus, dan alasan wajib diisi.');
        }
        ...
        if ($berlakuUntuk === 'Bulan') {
            if ($bulan < 1 || $bulan > 12 || $tahun <= 0) {
                return $this->model_response(false, 'Bulan berlaku tarif khusus wajib dipilih.');
            }
        ...
        if (!$rows) {
            return $this->model_response(false, 'Tagihan siswa pada periode yang dipilih tidak ditemukan.');
        }
        ...
        if ($nominal < $maxPaid) {
            return $this->model_response(false, 'Tarif khusus tidak boleh lebih kecil dari pembayaran yang sudah masuk.');
        }
        ...
        $this->db->insert('tagihan_keringanan_siswa', array(
            ...
            'alasan' => $alasan,
            'status' => 'Aktif',
            'tanggal_mulai' => $this->tanggal_sekarang(),
        ...
        $this->tagihan_log_activity(
            ...
            $sid,
            $master['kode_tagihan'],
            $first['nama_siswa'] . ' menjadi ' . $this->rupiah($nominal) . ' (' . $periodeLabel . ') - ' . $alasan,
            $first,
            array(
        ...
        return $this->tagihan_transaction_result('Tarif khusus siswa berhasil disimpan.');
    }
    ...
    public function kembalikan_normal()
    {
        ...
        if (!$specials) {
            return $this->model_response(false, 'Siswa tidak memiliki tarif khusus aktif.');
        }
    ...
    private function label_periode_tarif($latest, $jumlahAktif)
    {
        ...
        $label = $this->nama_bulan((int) $latest['bulan']) . ' ' . (int) $latest['tahun'];
        if ($jumlahAktif > 1) {
