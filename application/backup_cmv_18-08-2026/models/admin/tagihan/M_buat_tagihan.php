...
class M_buat_tagihan extends CI_Model
{
    ...
    private function input_data($tipe){
        ...
        $nama=trim((string)$this->input->post('nama_tagihan',true));$target=trim((string)$this->input->post('target_tagihan',true));$tunggakan=$this->input->post('dianggap_tunggakan',true)==='Tidak'?'Tidak':'Ya';$nominal=$this->nilai_nominal($this->input->post('nominal_default'));
        ...
        if($tipe==='Bulanan'){
            ...
            foreach($months as $m){$m=(int)$m;if($m<1||$m>12)continue;$year=$m>=7?$awal:$akhir;$val=isset($tarif[$m])?$this->nilai_nominal($tarif[$m]):$nominal;if($val<=0)$val=$nominal;if($val<=0)return array('error'=>'Nominal setiap bulan terpilih harus lebih dari nol.');$periods[]=array('bulan'=>$m,'tahun'=>$year,'nama_bulan'=>$this->nama_bulan($m),'nominal'=>$val,'jatuh_tempo'=>isset($jatuh[$m])?$jatuh[$m]:'');}
        }else{
            $month=(int)$this->input->post('bulan_penagihan');$year=(int)$this->input->post('tahun_penagihan');if($month<1||$month>12)$month=7;if(!$year)$year=$month>=7?$awal:$akhir;if($nominal<=0)return array('error'=>'Nominal tagihan harus lebih dari nol.');$periods[]=array('bulan'=>$month,'tahun'=>$year,'nama_bulan'=>$this->nama_bulan($month),'nominal'=>$nominal,'jatuh_tempo'=>trim((string)$this->input->post('tanggal_jatuh_tempo',true)));
        }
    ...
    public function preview($tipe){
        $data=$this->input_data($tipe);if(isset($data['error']))return $this->model_response(false,$data['error']);$students=$this->target_students($data);$total=0;foreach($data['periods'] as $p)$total+=count($students)*(float)$p['nominal'];return $this->model_response(true,'Preview berhasil.',array('jumlah_siswa'=>count($students),'jumlah_baris'=>count($students)*count($data['periods']),'total_nominal'=>$total,'periods'=>$data['periods'],'students'=>array_slice($students,0,20)));
    }

    public function simpan($tipe){
        $data=$this->input_data($tipe);if(isset($data['error']))return $this->model_response(false,$data['error']);$mode=$this->input->post('mode_simpan',true)==='Draft'?'Draft':'Terbitkan';$students=$this->target_students($data);if(!$students)return $this->model_response(false,'Tidak ada siswa aktif pada target yang dipilih.');
        $kode=$this->tagihan_next_code('TGH','tagihan_master','kode_tagihan');$first=$data['periods'][0];$last=end($data['periods']);reset($data['periods']);$audit=$this->tagihan_audit_fields();$master=array_merge(array('kode_tagihan'=>$kode,'id_jenis_tagihan'=>(int)$data['jenis']['id'],'nama_jenis_tagihan'=>$data['jenis']['nama_jenis'],'nama_tagihan'=>$data['nama'],'tipe_tagihan'=>$tipe,'id_periode'=>(int)$data['periode']['id'],'periode'=>$data['periode']['periode'],'semester'=>null,'nominal_default'=>$data['nominal'],'model_tarif_bulanan'=>$data['model_tarif'],'bulan_mulai'=>$first['bulan'],'tahun_mulai'=>$first['tahun'],'bulan_selesai'=>$last['bulan'],'tahun_selesai'=>$last['tahun'],'bulan_penagihan'=>$first['bulan'],'tahun_penagihan'=>$first['tahun'],'tanggal_jatuh_tempo'=>$first['jatuh_tempo'],'target_tagihan'=>$data['target'],'dianggap_tunggakan'=>$data['tunggakan'],'status_generate'=>$mode==='Draft'?'Belum':'Selesai','status'=>$mode==='Draft'?'Draft':'Aktif','keterangan'=>$data['keterangan']),$audit);
        ...
        $generated=0;$skipped=0;if($mode!=='Draft'){foreach($students as $s){foreach($data['periods'] as $p){
            ...
            $no=$this->tagihan_next_code('TAG','tagihan_siswa','no_tagihan');$nom=(float)$p['nominal'];$this->db->insert('tagihan_siswa',array('no_tagihan'=>$no,'id_tagihan_master'=>$idMaster,'kode_tagihan'=>$kode,'id_jenis_tagihan'=>(int)$data['jenis']['id'],'nama_jenis_tagihan'=>$data['jenis']['nama_jenis'],'nama_tagihan'=>$data['nama'],'tipe_tagihan'=>$tipe,'id_periode'=>(int)$data['periode']['id'],'periode'=>$data['periode']['periode'],'semester'=>null,'bulan'=>$p['bulan'],'nama_bulan'=>$p['nama_bulan'],'tahun'=>$p['tahun'],'tanggal_jatuh_tempo'=>$p['jatuh_tempo'],'id_siswa'=>(int)$s['id'],'nis'=>$s['nis'],'nisn'=>$s['nisn'],'nama_siswa'=>$s['nama_lengkap'],'id_kelas_setting'=>(int)$s['id_kelas_setting'],'id_kelas'=>(int)$s['id_kelas'],'nama_kelas'=>$s['nama_kelas'],'nominal_awal'=>$nom,'jenis_keringanan'=>null,'nilai_keringanan'=>0,'nominal_tagihan'=>$nom,'nominal_dibayar'=>0,'sisa_tagihan'=>$nom,'dianggap_tunggakan'=>$data['tunggakan'],'status_pembayaran'=>'Belum Dibayar','status_tagihan'=>'Aktif','keterangan'=>$data['keterangan'],'tanggal_generate'=>$this->tanggal_sekarang(),'waktu_generate'=>$this->waktu_sekarang(),'id_user_generate'=>$this->app_user_id(),'nama_user_generate'=>$this->app_user_name()));$generated++;}}}
        $this->tagihan_log_activity($mode==='Draft'?'Simpan Draft Tagihan':'Terbitkan Tagihan','Tagihan',$mode==='Draft'?'Tambah':'Tambah','tagihan_master',$idMaster,$kode,$data['nama'].' - '.$generated.' tagihan siswa',null,$master);$res=$this->tagihan_transaction_result($mode==='Draft'?'Draft tagihan berhasil disimpan.':$generated.' tagihan siswa berhasil diterbitkan'.($skipped?' dan '.$skipped.' duplikat dilewati.':'.'));if($res['result']==='true')$res['id']=$idMaster;return $res;
    }
