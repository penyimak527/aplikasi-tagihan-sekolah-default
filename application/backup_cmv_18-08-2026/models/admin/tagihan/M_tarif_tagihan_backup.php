...
class M_tarif_tagihan extends CI_Model
{
    ...
    public function simpan_kelas(){
        $id=(int)$this->input->post('id_tagihan');$tarif=$this->input->post('tarif');if(!is_array($tarif))$tarif=array();$m=$this->db->where('id',$id)->get('tagihan_master')->row_array();if(!$m||!$tarif)return $this->model_response(false,'Data tarif tidak lengkap.');$this->db->trans_begin();foreach($tarif as $idTarget=>$nominal){$idTarget=(int)$idTarget;$nominal=$this->nilai_nominal($nominal);if($nominal<0){$this->db->trans_rollback();return $this->model_response(false,'Tarif tidak boleh negatif.');}$target=$this->db->where('id',$idTarget)->where('id_tagihan_master',$id)->get('tagihan_target_kelas')->row_array();if(!$target)continue;$maxPaid=(float)$this->db->select_max('nominal_dibayar','max')->where('id_tagihan_master',$id)->where('id_kelas_setting',$target['id_kelas_setting'])->get('tagihan_siswa')->row()->max;if($nominal<$maxPaid){$this->db->trans_rollback();return $this->model_response(false,'Tarif kelas '.$target['nama_kelas'].' tidak boleh lebih kecil dari pembayaran yang sudah masuk.');}$before=$target;$this->db->where('id',$idTarget)->update('tagihan_target_kelas',array('nominal_kelas'=>$nominal,'tanggal'=>$this->tanggal_sekarang(),'waktu'=>$this->waktu_sekarang(),'id_user'=>$this->app_user_id(),'nama_user'=>$this->app_user_name()));$rows=$this->db->where('id_tagihan_master',$id)->where('id_kelas_setting',$target['id_kelas_setting'])->get('tagihan_siswa')->result_array();foreach($rows as $row){$special=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$row['id_siswa'])->where('jenis_keringanan','Tarif Khusus')->where('status','Aktif')->order_by('id','DESC')->get('tagihan_keringanan_siswa')->row_array();if($special)continue;$newSisa=max(0,$nominal-(float)$row['nominal_dibayar']);$status=$newSisa<=0?'Lunas':((float)$row['nominal_dibayar']>0?'Dibayar Sebagian':'Belum Dibayar');$this->db->where('id',$row['id'])->update('tagihan_siswa',array('nominal_awal'=>$nominal,'nominal_tagihan'=>$nominal,'sisa_tagihan'=>$newSisa,'status_pembayaran'=>$status,'tanggal_update'=>$this->tanggal_sekarang(),'waktu_update'=>$this->waktu_sekarang()));}$this->tagihan_log_activity('Ubah Tarif Kelas','Tagihan','Ubah','tagihan_target_kelas',$idTarget,$m['kode_tagihan'],$target['nama_kelas'].' menjadi '.$this->rupiah($nominal),$before,array('nominal_kelas'=>$nominal));}
    ...
    public function simpan_siswa(){
        $id=(int)$this->input->post('id_tagihan');$sid=(int)$this->input->post('id_siswa');$nominal=$this->nilai_nominal($this->input->post('nominal_khusus'));$alasan=trim((string)$this->input->post('alasan',true));$m=$this->db->where('id',$id)->get('tagihan_master')->row_array();$s=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_siswa')->row_array();if(!$m||!$s||$alasan===''||$nominal<0)return $this->model_response(false,'Siswa, nominal khusus, dan alasan wajib diisi.');$maxPaid=(float)$this->db->select_max('nominal_dibayar','max')->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_siswa')->row()->max;if($nominal<$maxPaid)return $this->model_response(false,'Tarif khusus tidak boleh lebih kecil dari pembayaran yang sudah masuk.');
        $this->db->trans_begin();$this->db->where('id_tagihan_master',$id)->where('id_siswa',$sid)->where('jenis_keringanan','Tarif Khusus')->where('status','Aktif')->update('tagihan_keringanan_siswa',array('status'=>'Dibatalkan','tanggal_batal'=>$this->tanggal_sekarang(),'waktu_batal'=>$this->waktu_sekarang(),'id_user_batal'=>$this->app_user_id(),'nama_user_batal'=>$this->app_user_name(),'alasan_batal'=>'Diganti tarif khusus baru'));
        $this->db->insert('tagihan_keringanan_siswa',array('id_tagihan_master'=>$id,'id_siswa'=>$sid,'nis'=>$s['nis'],'nisn'=>$s['nisn'],'nama_siswa'=>$s['nama_siswa'],'bulan'=>0,'tahun'=>0,'jenis_keringanan'=>'Tarif Khusus','nominal_awal'=>$s['nominal_awal'],'nilai_keringanan'=>max(0,(float)$s['nominal_awal']-$nominal),'nominal_setelah_keringanan'=>$nominal,'alasan'=>$alasan,'status'=>'Aktif','tanggal_mulai'=>$this->tanggal_sekarang(),'tanggal'=>$this->tanggal_sekarang(),'waktu'=>$this->waktu_sekarang(),'id_user'=>$this->app_user_id(),'nama_user'=>$this->app_user_name()));
        $target=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_target_siswa')->row_array();$targetData=array('id_tagihan_master'=>$id,'id_siswa'=>$sid,'nis'=>$s['nis'],'nisn'=>$s['nisn'],'nama_siswa'=>$s['nama_siswa'],'id_kelas_setting'=>$s['id_kelas_setting'],'id_kelas'=>$s['id_kelas'],'nama_kelas'=>$s['nama_kelas'],'nominal_target'=>$nominal,'status'=>'Aktif','tanggal'=>$this->tanggal_sekarang(),'waktu'=>$this->waktu_sekarang(),'id_user'=>$this->app_user_id(),'nama_user'=>$this->app_user_name());if($target)$this->db->where('id',$target['id'])->update('tagihan_target_siswa',$targetData);else$this->db->insert('tagihan_target_siswa',$targetData);
        $rows=$this->db->where('id_tagihan_master',$id)->where('id_siswa',$sid)->get('tagihan_siswa')->result_array();foreach($rows as $row){$sisa=max(0,$nominal-(float)$row['nominal_dibayar']);$status=$sisa<=0?'Lunas':((float)$row['nominal_dibayar']>0?'Dibayar Sebagian':'Belum Dibayar');$this->db->where('id',$row['id'])->update('tagihan_siswa',array('jenis_keringanan'=>'Tarif Khusus','nilai_keringanan'=>max(0,(float)$row['nominal_awal']-$nominal),'nominal_tagihan'=>$nominal,'sisa_tagihan'=>$sisa,'status_pembayaran'=>$status,'tanggal_update'=>$this->tanggal_sekarang(),'waktu_update'=>$this->waktu_sekarang()));}
        $this->tagihan_log_activity('Tarif Khusus Siswa','Tagihan','Ubah','tagihan_siswa',$sid,$m['kode_tagihan'],$s['nama_siswa'].' menjadi '.$this->rupiah($nominal).' - '.$alasan,$s,array('nominal_khusus'=>$nominal));return $this->tagihan_transaction_result('Tarif khusus siswa berhasil disimpan.');
    }
    ...
    public function kembalikan_normal()
    {
        ...
        if (!$master || !$rows) {
            return $this->model_response(false, 'Tagihan atau siswa tidak ditemukan.');
        }
        if ($alasan === '') {
        ...
        foreach ($rows as $row) {
            ...
            if ($normal < $dibayar) {
                ...
                return $this->model_response(false, 'Tarif normal tidak boleh lebih kecil dari pembayaran yang sudah masuk.');
            }
        ...
        return $this->tagihan_transaction_result('Tarif siswa berhasil dikembalikan ke tarif normal.');
    }
