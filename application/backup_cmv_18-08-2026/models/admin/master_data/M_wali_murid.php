...
class M_wali_murid extends CI_Model
{
    ...
    public function detail()
    {
        ...
        return $this->model_response(true, 'Detail wali murid berhasil dimuat.', array(
            'data' => $wali,
            'relasi' => $relasi
        ));
    }
    ...
    public function generate_username()
    {
        ...
        if ($nama === '') {
            return $this->model_response(false, 'Nama wali harus diisi terlebih dahulu.');
        }
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            if ($this->db->where('username', $username)->count_all_results('wali_murid') === 0) {
                return $this->model_response(true, 'Username berhasil dibuat.', array('username' => $username));
            }
        }

        return $this->model_response(false, 'Username unik gagal dibuat. Silakan coba lagi.');
    }

    public function generate_password()
    {
        return $this->model_response(true, 'Password berhasil dibuat.', array(
    ...
    public function simpan()
    {
        ...
        if ($validasi !== true) {
            return $this->model_response(false, $validasi);
        }
        $validasiPassword = $this->validasi_password($password);
        if ($validasiPassword !== true) {
            return $this->model_response(false, $validasiPassword);
        }

        $siswa = $this->parse_siswa_json($this->input->post('siswa_json'));
        $kode = $this->tagihan_next_code('WALI', 'wali_murid', 'kode_wali');
        $data = array(
            'kode_wali' => $kode,
            'nama_wali' => $nama,
            'no_telepon' => $telepon,
        ...
        $result = $this->tagihan_transaction_result('Akun wali murid berhasil dibuat.');
    ...
    public function update()
    {
        ...
        return $this->tagihan_transaction_result('Data wali murid berhasil diperbarui.');
    }
    ...
    public function status()
    {
        ...
        if (!in_array($target, array('Aktif', 'Tidak Aktif'), true)) {
            return $this->model_response(false, 'Status akun tidak valid.');
        }
        ...
        if ($before['status'] === $target) {
            return $this->model_response(true, 'Status akun sudah ' . $target . '.');
        }
        ...
        return $this->tagihan_transaction_result('Akun wali murid berhasil ' . ($target === 'Aktif' ? 'diaktifkan.' : 'dinonaktifkan.'));
    }
    ...
    public function tambah_relasi()
    {
        ...
        if (!$wali) {
            return $this->model_response(false, 'Akun wali murid tidak ditemukan.');
        }

        $siswa = $this->parse_siswa_json($this->input->post('siswa_json'));
        if (!$siswa) {
            return $this->model_response(false, 'Pilih minimal satu siswa.');
        }
        ...
        $result = $this->tagihan_transaction_result($jumlah . ' relasi siswa berhasil disimpan.');
    ...
    public function ubah_relasi()
    {
        ...
        if (!$before) {
            return $this->model_response(false, 'Relasi wali dan siswa tidak ditemukan.');
        }
        ...
        return $this->tagihan_transaction_result('Hubungan wali dengan siswa berhasil diperbarui.');
    }
    ...
    public function status_relasi()
    {
        ...
        $target = trim((string) $this->input->post('status', true));
        if (!in_array($target, array('Aktif', 'Tidak Aktif'), true)) {
            return $this->model_response(false, 'Status relasi tidak valid.');
        }
        ...
        return $this->tagihan_transaction_result('Relasi siswa berhasil ' . ($target === 'Aktif' ? 'diaktifkan.' : 'dinonaktifkan.'));
    }
    ...
    public function reset_password()
    {
        ...
        $result = $this->tagihan_transaction_result('Password wali murid berhasil direset.');
        if ($result['result'] === 'true') {
            $result['credential'] = array(
