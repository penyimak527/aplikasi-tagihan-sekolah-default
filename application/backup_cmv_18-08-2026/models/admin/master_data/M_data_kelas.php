...
class M_data_kelas extends CI_Model
{
    ...
    public function detail(){
        ...
        if(!$row)return $this->model_response(false,'Kelas tidak ditemukan.');
    ...
    public function simpan(){
        ...
        if($nama==='')return $this->model_response(false,'Nama kelas wajib diisi.');if($status==='')$status='REGULER';
        ...
        $this->tagihan_log_activity($before?'Ubah Kelas':'Tambah Kelas','Master Data',$before?'Ubah':'Tambah','kelas',$id,$nama,'Pengelolaan master kelas',$before,$data);
        return $this->tagihan_transaction_result('Data kelas berhasil disimpan.');
    }
    public function hapus(){
        $id=(int)$this->input->post('id');$row=$this->db->where('id',$id)->get('kelas')->row_array();if(!$row)return $this->model_response(false,'Data tidak ditemukan.');
        if($this->db->where('id_kelas',(string)$id)->count_all_results('kelas_setting'))return $this->model_response(false,'Kelas sudah digunakan pada pengaturan kelas dan tidak dapat dihapus.');
        $this->db->trans_begin();$this->db->where('id',$id)->delete('kelas');$this->tagihan_log_activity('Hapus Kelas','Master Data','Batal','kelas',$id,$row['nama_kelas'],'Menghapus kelas yang belum digunakan',$row,null);return $this->tagihan_transaction_result('Kelas berhasil dihapus.');
    }
