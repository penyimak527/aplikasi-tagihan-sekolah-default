...
class M_riwayat_kelas extends CI_Model
{
    ...
    public function koreksi()
    {
        ...
        if (!$idSiswa || !$idTujuan || $alasan === '') {
            return $this->model_response(false, 'Siswa, kelas koreksi, dan alasan wajib diisi.');
        }
        ...
        if (!$siswa || !$tujuan || !$asal) {
            return $this->model_response(false, 'Data siswa atau penempatan aktif tidak ditemukan.');
        }
        if ((int) $asal['id'] === $idTujuan) {
            return $this->model_response(false, 'Kelas koreksi sama dengan kelas aktif saat ini.');
        }
        ...
        $result = $this->tagihan_transaction_result('Penempatan terakhir berhasil dikoreksi.');
