...
class M_pengaturan extends CI_Model
{
    ...
    public function format_save($jenis)
    {
        ...
        if ($name === '') return $this->model_response(false, 'Nama format wajib diisi.');
        $before = $id ? $this->format_detail($id) : null;
        $logo = $before['logo_sekolah'] ?? '';
        if (!empty($_FILES['logo']['name'])) {
            $ext = strtolower(pathinfo($_FILES['logo']['name'], PATHINFO_EXTENSION));
            if (!in_array($ext, array('png', 'jpg', 'jpeg', 'webp'), true)) return $this->model_response(false, 'Logo hanya boleh PNG, JPG, JPEG, atau WEBP.');
            $dir = FCPATH . 'uploads/pengaturan/';
            if (!is_dir($dir)) mkdir($dir, 0755, true);
            $file = 'logo_' . date('YmdHis') . '_' . mt_rand(100, 999) . '.' . $ext;
            if (!move_uploaded_file($_FILES['logo']['tmp_name'], $dir . $file)) return $this->model_response(false, 'Upload logo gagal.');
        ...
        if (is_string($json) && $json !== '' && json_decode($json, true) === null) return $this->model_response(false, 'Pengaturan khusus harus berupa JSON yang valid.');
        ...
        if ($id) {
            ...
            $data = array_merge($data, $this->tagihan_audit_fields());
            $this->db->insert('tagihan_pengaturan_cetak', $data);
            $id = $this->db->insert_id();
        }
        $this->tagihan_log_activity(($before ? 'Ubah' : 'Tambah') . ' Format ' . $jenis, 'Pengaturan', $before ? 'Ubah' : 'Tambah', 'tagihan_pengaturan_cetak', $id, $name, 'Konfigurasi format ' . $jenis, $before, $data);
        return $this->tagihan_transaction_result('Pengaturan format berhasil disimpan.');
    }
    public function set_default()
    {
        $id = (int)$this->input->post('id');
        $r = $this->format_detail($id);
        if (!$r) return $this->model_response(false, 'Format tidak ditemukan.');
        $this->db->trans_begin();
        ...
        $this->tagihan_log_activity('Jadikan Format Default', 'Pengaturan', 'Ubah', 'tagihan_pengaturan_cetak', $id, $r['nama_format'], 'Format default ' . $r['jenis_format'], $r, array('status_default' => 'Ya'));
        return $this->tagihan_transaction_result('Format berhasil dijadikan default.');
    }
    ...
    public function template_save()
    {
        ...
        $status = $this->input->post('status', true) === 'Tidak Aktif' ? 'Tidak Aktif' : 'Aktif';
        if ($default === 'Ya') $status = 'Aktif';
        if (!in_array($jenis, array('Bukti Pembayaran', 'Surat Tunggakan'), true) || $nama === '' || $isi === '') return $this->model_response(false, 'Jenis, nama, dan isi template wajib diisi.');
        ...
        if ($unknown) return $this->model_response(false, 'Variabel tidak dikenal: ' . implode(', ', $unknown));
        ...
        $this->tagihan_log_activity(($before ? 'Ubah' : 'Tambah') . ' Template WhatsApp', 'Pengaturan', $before ? 'Ubah' : 'Tambah', 'tagihan_template_whatsapp', $id, $nama, 'Template ' . $jenis, $before, $data);
        return $this->tagihan_transaction_result('Template WhatsApp berhasil disimpan.');
    }
    public function template_default()
    {
        $id = (int)$this->input->post('id');
        $r = $this->db->where('id', $id)->get('tagihan_template_whatsapp')->row_array();
        if (!$r) return $this->model_response(false, 'Template tidak ditemukan.');
        $this->db->trans_begin();
        ...
        $this->tagihan_log_activity('Jadikan Template Default', 'Pengaturan', 'Ubah', 'tagihan_template_whatsapp', $id, $r['nama_template'], 'Template default ' . $r['jenis_template']);
        return $this->tagihan_transaction_result('Template berhasil dijadikan default.');
    }
