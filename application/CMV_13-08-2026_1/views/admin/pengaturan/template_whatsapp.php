...
function loadData() {
    $.getJSON('<?= base_url('admin/pengaturan/template_whatsapp/result') ?>', function (response) {
...
function saveData() {
    ...
    $.post('<?= base_url('admin/pengaturan/template_whatsapp/simpan') ?>', $('#form').serialize(), function (response) {
...
function setDefault(id) {
    confirmAction('Jadikan template default?', 'Template default pada jenis yang sama akan diganti.', function () {
        $.post('<?= base_url('admin/pengaturan/template_whatsapp/set_default') ?>', {id: id}, function (response) {
            var ok = response.result === 'true';
            Swal.fire(ok ? 'Berhasil' : 'Gagal', response.message, ok ? 'success' : 'error');
            if (ok) loadData();
        }, 'json').fail(ajaxError);
