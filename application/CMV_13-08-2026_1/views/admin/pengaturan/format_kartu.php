...
$(function () {
    ...
    function load() {
        $.getJSON('<?= base_url('admin/pengaturan/format_kartu/result') ?>', function (response) {
            rows = response || [];
            if (!rows.length) {
    ...
    $(document).on('click', '.def', function () {
        ...
        confirmAction('Jadikan format default?', 'Format default sebelumnya akan diganti.', function () {
            $.post('<?= base_url('admin/pengaturan/format_kartu/set_default') ?>', {id: id}, function (response) {
    ...
    $('#form').on('submit', function (event) {
        ...
        $.post('<?= base_url('admin/pengaturan/format_kartu/simpan') ?>', $(this).serialize(), function (response) {
            var ok = response.result === 'true';
            Swal.fire(ok ? 'Berhasil' : 'Gagal', response.message, ok ? 'success' : 'error');
            if (ok) {
                resetForm();
