            ...
            <div class="card-body">
                ...
                <div id="preview_area" class="mt-4 d-none">
                    <div class="row g-2 mb-3">
                        ...
                        <div class="col-md-3">
                            <div class="alert alert-warning mb-0">Duplikat: <strong id="sum_duplikat">0</strong></div>
                        </div>
...
<script>
    ...
    function importData() {
        confirmAction('Import data valid?', 'Baris gagal tidak akan disimpan.', function() {
            ...
            $.ajax({
                url: '<?= base_url('admin/master_data/import_siswa/proses') ?>',
                type: 'POST',
                data: {
                    token: $('#token').val()
                },
                ...
                success: function(r) {
                    Swal.fire(
                        r.result === 'true' ? 'Berhasil' : 'Gagal',
                        r.message,
                        r.result === 'true' ? 'success' : 'error'
                    );
    ...
    function loadRiwayat() {
        $.ajax({
            url: '<?= base_url('admin/master_data/import_siswa/riwayat') ?>',
            type: 'POST',
            data: {},
            dataType: 'JSON',
            success: function(rows) {
                ...
                rows.forEach(function(r) {
                    ...
                    var download = gagal > 0
                        ? '<div class="mt-2"><a class="btn btn-sm btn-outline-danger" href="<?= base_url('admin/master_data/import_siswa/download_gagal/') ?>' + Number(r.id) + '"><i class="ri-download-line me-1"></i>Unduh Data Gagal</a></div>'
                        : '';

                    h += '<div class="riwayat-import-item border-bottom py-3">' +
                        ...
                        '<small class="text-muted">' +
                            ...
                            escapeHtml(r.periode || '-') + ' | ' + escapeHtml(r.nama_kelas) + '<br>' +
