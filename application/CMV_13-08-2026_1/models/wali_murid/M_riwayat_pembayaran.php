...
class M_riwayat_pembayaran extends CI_Model
{
    ...
    public function detail($id_wali, $id_pembayaran)
    {
        ...
        if (!$header) {
            return model_response(false, 'Transaksi pembayaran tidak ditemukan.');
        }
        ...
        if (!$valid) {
            return model_response(false, 'Transaksi tidak dapat diakses oleh akun ini.');
        }
        ...
        return model_response(true, 'Detail pembayaran berhasil dimuat.', array(
            'header' => $header,
            'detail' => $detail,
