...
class M_daftar_tagihan extends CI_Model
{
    ...
    public function draft_detail()
    {
        ...
        if (!$master) {
            return model_response(false, 'Draft tagihan tidak ditemukan.');
        }
        if ($master['status'] !== 'Draft') {
            return model_response(false, 'Hanya tagihan berstatus Draft yang dapat diedit.');
        }
        if ($this->db->where('id_tagihan_master', $id)->count_all_results('tagihan_siswa') > 0) {
            return model_response(false, 'Draft sudah memiliki tagihan siswa dan tidak dapat diedit dari halaman ini.');
        }

        return model_response(true, 'Draft berhasil dimuat.', array(
            'master' => $master,
            'periods' => $this->db
    ...
    public function update_draft()
    {
        ...
        if ($master['status'] !== 'Draft') {
            return model_response(false, 'Hanya tagihan berstatus Draft yang dapat diubah.');
        }
        if ($this->db->where('id_tagihan_master', $id)->count_all_results('tagihan_siswa') > 0) {
            return model_response(false, 'Draft sudah memiliki tagihan siswa dan tidak dapat diubah.');
        }
        ...
        if ($nama === '') {
            return model_response(false, 'Nama tagihan wajib diisi.');
        }
        if (!is_array($periods) || !count($periods)) {
            return model_response(false, 'Minimal satu periode tarif harus tersedia.');
        }
        ...
        foreach ($periods as $row) {
            ...
            if ($periodId <= 0 || !isset($storedMap[$periodId])) {
                ...
                return model_response(false, 'Periode draft tidak valid.');
            }
            ...
            if ($nominal <= 0) {
                $this->db->trans_rollback();
                return model_response(false, 'Nominal setiap periode harus lebih dari nol.');
            }
        ...
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            return model_response(false, 'Draft tagihan gagal diperbarui.');
        }

        $this->db->trans_commit();
        return model_response(true, 'Draft tagihan berhasil diperbarui.');
    }
    ...
    public function hapus_draft()
    {
        ...
        if ($master['status'] !== 'Draft') {
            return model_response(false, 'Hanya tagihan berstatus Draft yang dapat dihapus.');
        }
        if ($this->db->where('id_tagihan_master', $id)->count_all_results('tagihan_siswa') > 0) {
            return model_response(false, 'Draft sudah memiliki tagihan siswa sehingga tidak dapat dihapus.');
        }
        ...
        if ($this->db->trans_status() === false) {
            $this->db->trans_rollback();
            return model_response(false, 'Draft tagihan gagal dihapus.');
        }

        $this->db->trans_commit();
        return model_response(true, 'Draft tagihan berhasil dihapus.');
    }
