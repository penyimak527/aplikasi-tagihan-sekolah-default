...
class M_import_siswa extends CI_Model
{
    ...
    public function preview()
    {
        ...
        if (!$periode || !$kelas || ((int) $kelas['id_periode'] !== $id_periode))
            return model_response(false, 'Tahun ajaran dan kelas penempatan tidak sesuai.');
        if (empty($_FILES['file_excel']['name']))
            return model_response(false, 'Pilih file XLSX terlebih dahulu.');
        $ext = strtolower(pathinfo($_FILES['file_excel']['name'], PATHINFO_EXTENSION));
        if ($ext !== 'xlsx')
            return model_response(false, 'File yang diterima hanya format .xlsx.');
        ...
        if (!move_uploaded_file($_FILES['file_excel']['tmp_name'], $path))
            return model_response(false, 'File gagal diunggah.');
        try {
        ...
        if (count($rows) < 2) {
            @unlink($path);
            return model_response(false, 'File tidak memiliki data siswa.');
        }
        ...
        foreach ($required as $h)
            if (!in_array($h, $headers, true)) {
                @unlink($path);
                return model_response(false, 'Kolom wajib ' . $h . ' tidak ditemukan.');
            }
        $seenNis = array();
        $seenNisn = array();
        $preview = array();
        ...
        return model_response(true, 'Preview berhasil dibuat.', array(
            'token' => $token,
            'rows' => $preview,
            'total' => count($preview),
            'valid' => $valid,
    ...
    public function proses()
    {
        ...
        if (!$session)
            return model_response(false, 'Data preview tidak ditemukan atau sudah kedaluwarsa.');
        $validRows = array_values(array_filter($session['rows'], function ($r) {
            return $r['status'] === 'Valid'; }));
        if (!$validRows)
            return model_response(false, 'Tidak ada data valid yang dapat diimport.');
        $kode = 'IMP/' . date('Ym') . '/' . str_pad(((int) $this->db->count_all('tagihan_import_siswa')) + 1, 5, '0', STR_PAD_LEFT);
        $audit = tagihan_audit_fields();
        ...
        $result = tagihan_transaction_result('Import selesai. ' . $success . ' siswa berhasil disimpan.');
