...
class M_kelas_setting extends CI_Model
{
    ...
    public function simpan()
    {
        ...
        if (!$idPeriode || !$idKelas) {
            return model_response(false, 'Tahun ajaran dan kelas wajib dipilih.');
        }

        $periode = $this->db->where('id', $idPeriode)->get('master_tahun_ajaran')->row_array();
        $kelas = $this->db->where('id', $idKelas)->get('kelas')->row_array();
        if (!$periode || !$kelas) {
            return model_response(false, 'Tahun ajaran atau kelas tidak ditemukan pada master.');
        }
        ...
        if ($duplicate > 0) {
            return model_response(false, 'Kombinasi tahun ajaran dan kelas sudah tersedia.');
        }

        $before = $id ? $this->db->where('id', $id)->get('kelas_setting')->row_array() : null;
        if ($id && !$before) return model_response(false, 'Kelas Setting tidak ditemukan.');
        ...
        return tagihan_transaction_result($preserveHistory
    ...
    public function hapus()
    {
        ...
        if (!$row) return model_response(false, 'Kelas Setting tidak ditemukan.');

        if ($this->db->where('id_kelas_setting', (string)$id)->count_all_results('kelas_siswa') > 0) {
            return model_response(false, 'Kelas Setting sudah digunakan pada penempatan siswa dan tidak dapat dihapus.');
        }

        $this->db->trans_begin();
        $this->db->where('id', $id)->delete('kelas_setting');
        tagihan_log_activity('Hapus Kelas Setting', 'Master Data', 'Batal', 'kelas_setting', $id, $row['periode'].' - '.$row['nama_kelas'], 'Menghapus Kelas Setting yang belum digunakan', $row, null);
        return tagihan_transaction_result('Kelas Setting berhasil dihapus.');
    }
}
